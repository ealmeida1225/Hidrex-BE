...
const url = "/business-gestion/precipitation-concentration-index-monthly-by-pluviometer/";
      ...
      buttons: [
        {
          ...
          action: function (e, dt, node, config) {
            $("#modal-create-precipitation-concentration-index-monthy-by-pluviometer").modal("show");
          },
        },
        {
          extend: "excel",
...

        {
          ...
          render: (data, type, row) => {
            return `<div class="btn-group">
                        <button type="button" title="edit" class="btn bg-olive active" data-toggle="modal" data-target="#modal-create-precipitation-concentration-index-monthy-by-pluviometer" data-id="${row.id}" data-type="edit" data-name="${row.name}" id="${row.id}"  >
...
$("#modal-create-precipitation-concentration-index-monthy-by-pluviometer").on("hide.bs.modal", (event) => {
...
$("#modal-create-precipitation-concentration-index-monthy-by-pluviometer").on("show.bs.modal", function (event) {
...
$(function () {
  ...
  $("#form-create-precipitation-concentration-index-monthy-by-pluviometer").validate({
    rules: {
      name: {
        required: true,
      },
...
let form = document.getElementById("form-create-precipitation-concentration-index-monthy-by-pluviometer");
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          $("#modal-create-precipitation-concentration-index-monthy-by-pluviometer").modal("hide");
          Swal.fire({
            icon: "success",
